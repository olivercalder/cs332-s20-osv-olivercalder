...
main()
{
    ...
    if ((i = read(fd, buf, 10)) != 10) {
        error("read of first 10 bytes unsuccessful was %d bytes", i);
    }
    ...
    if ((i = read(fd, buf, 10)) != 10) {
        error("read of second 10 bytes unsuccessful was %d bytes", i);
    }
    ...
    if ((i = read(fd, buf, 10)) != 6) {
        error("read of last 6 bytes unsuccessful was %d bytes", i);
    }
